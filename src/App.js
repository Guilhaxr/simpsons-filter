...
function App() {
  ...
  return (
    <div>
      <button onClick= { () => {setSimpsonsOnly(!simpsonsOnly)} } >Simpsons family Only: {simpsonsOnly ? 'ON' : 'OFF' }</button>
      {quoteList.filter((item) => !simpsonsOnly || item.character.includes('Simpson')).map((quote, index) => (
